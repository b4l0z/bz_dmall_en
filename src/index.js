...
const client = new Discord.Client({
  intents: Object.keys(Discord.GatewayIntentBits).map((a) => {
    if (isNaN(a)) console.log(yellowBright(`SETUP | Intent`), magenta(a), yellowBright` loading...`);
    return Discord.GatewayIntentBits[a];
  }),
  partials: Object.keys(Discord.Partials).map((b) => {
    if (isNaN(b)) console.log(yellowBright(`SETUP | Partial`), magenta(b), yellowBright` loading...`);
...
client.on("ready", () => {
  bzdmall();
  console.log(green(`CLIENT | Operational`));
  client.channels.cache
    .get(config.id.ping_test)
    .send("Message to test bot ping.")
    .then((msg) => {
